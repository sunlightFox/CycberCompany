...
本文件保留的是项目早期总路线图与工程约束，适合回答：

```text
项目为什么这样分层
开发顺序为什么是 schema -> migration -> service -> api -> tests
Codex 在仓库里必须遵守哪些硬边界
```

随着项目进入高编号增量阶段，详细阶段拆解已经转移到 `docs/开发计划/` 目录维护。

当前阅读顺序建议为：

```text
先读本文件理解全局原则
再读 docs/开发计划/README.md 查看最新阶段索引
再进入对应阶段文档，例如 101 / 102 / 103
```

当前与最新代码现实直接相关的后续阶段锚点：

```text
101 扩展兼容层与渠道平台插件化闭环
102 视频理解剪辑执行域聚焦打深与媒体工作流闭环
103 闭环成功率度量体系与真实任务门禁
```
