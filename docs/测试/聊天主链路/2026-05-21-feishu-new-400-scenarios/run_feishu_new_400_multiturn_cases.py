...
REPORT_PATH = BASE_DIR / "02-飞书400个新增多轮场景测试执行报告.md"
CASESET_PATH = BASE_DIR / "01-测试用例-飞书400个新增多轮场景.md"


def _load_base100() -> Any:
    spec = importlib.util.spec_from_file_location("feishu_new_400_base100", BASE_100_PATH)
...
def _mk(
    ...
    return EC(f"FHN400-{case_no:03d}", category, title, peer_ref, prompt, checker)
...
def _all_cases(site: Any) -> list[Any]:
    cases: list[Any] = []
    _common_category_cases(cases)
    _memory_category_cases(cases)
    _search_timeliness_cases(cases)
    _browser_cases(cases, site)
    _office_cases(cases)
    _system_schedule_host_cases(cases)
    _skill_mcp_cases(cases)
    _popular_life_cases(cases)
    _mixed_followup_cases(cases)
    if len(cases) != 400:
...
def run() -> list[Any]:
    ...
    os.environ["FEISHU_APP_ID"] = "feishu-new-400-app"
    os.environ["FEISHU_APP_SECRET"] = "feishu-new-400-secret"
