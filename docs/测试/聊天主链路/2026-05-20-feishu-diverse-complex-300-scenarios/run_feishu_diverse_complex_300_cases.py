...
REPORT_PATH = BASE_DIR / "02-飞书300轮多样复杂场景测试执行报告.md"
CASESET_PATH = BASE_DIR / "01-测试用例-飞书300轮多样复杂场景.md"


def _load_base100() -> Any:
    spec = importlib.util.spec_from_file_location("feishu_diverse_complex_300_base100", BASE_100_PATH)
...
def _mk(case_no: int, category: str, title: str, peer_ref: str, prompt: str, checker: Callable[[Any, TestClient, dict[str, Any]], list[str]]) -> Any:
    return EC(f"FHD300-{case_no:03d}", category, title, peer_ref, prompt, checker)
...
def run() -> list[Any]:
    ...
    os.environ["FEISHU_APP_ID"] = "feishu-diverse-complex-300-app"
    os.environ["FEISHU_APP_SECRET"] = "feishu-diverse-complex-300-secret"
