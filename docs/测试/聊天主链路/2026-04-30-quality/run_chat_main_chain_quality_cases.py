...
RUN_LABEL = "CHAT-E2E-20260430-QUALITY"
...
REPORT_PATH = TEST_DIR / "07-高质量体验测试执行报告.md"
ISSUES_PATH = TEST_DIR / "08-高质量体验待修复问题.md"
TABLE_PATH = TEST_DIR / "09-聊天输入回复质量总表.md"

TEST_USERNAME = "chat-e2e-quality-user"
SECRET_PREFIX = "s" + "k-"
TEST_PASSWORD = "CHAT-E2E-20260430-QUALITY-login-" + "password"
WRONG_PASSWORD = "CHAT-E2E-20260430-QUALITY-wrong-" + "password"
LOGIN_CODE = "ok-quality"
URL_SECRET = SECRET_PREFIX + "chatmaine2e-quality-url-secret"
TERMINAL_SECRET = SECRET_PREFIX + "qualityterminalsecret123"
PRIVACY_SECRET = SECRET_PREFIX + "chatmaine2e-quality-privacy-secret"
PRIVACY_PASSWORD = "quality-privacy-" + "password"
MEMORY_PASSWORD = "CHAT-E2E-20260430-QUALITY-memory-" + "password"
...
class QualityRunner:
    ...
    def _run_task_direct_case(self, client: TestClient, case_id: str, title: str, runner: str) -> CaseResult:
        if runner in {"file_write", "file_read", "file_hash", "task_replay"}:
        ...
        if runner == "terminal_echo":
            ...
            response = self._execute_tool(client, task.get("task_id"), "terminal.run", {"command": "echo CHAT-E2E-20260430-QUALITY"}, approve=True)
    ...
    def _run_browser_case(self, client: TestClient, case_id: str, title: str, runner: str) -> CaseResult:
        ...
        if runner == "chat_browser_search":
            return self._run_chat_case(client, {"case_id": case_id, "category": "浏览器桌面系统执行验证", "title": title, "turns": [f"{RUN_LABEL} 浏览器：请用浏览器搜索 CHAT-E2E-20260430-QUALITY browser search，并总结结果，必须说明证据来源。"], "checks": ["completed", "task_created_or_boundary", "no_secret_leak"], "quality_min": 8})
...
""".strip(), encoding="utf-8")
    ...
    def _fail_case(self, result: CaseResult, severity: str, title: str, expected: str, actual: str, evidence: dict[str, Any] | None = None) -> Issue:
        if result.status != "BLOCKED":
            result.status = "FAIL"
        self.issue_index += 1
        issue = Issue(issue_id=f"CHAT-E2E-QUALITY-FIX-{self.issue_index:03d}", severity=severity, case_id=result.case_id, title=title, expected=expected, actual=str(redact_value(actual))[:4000], evidence=redact_value(evidence or {}))
    ...
    def _render_issues(self) -> str:
        lines = ["# 聊天主链路高质量体验待修复问题", "", f"- 测试批次：`{RUN_LABEL}`", f"- 运行 ID：`{RUN_ID}`", f"- 问题总数：{len(self.issues)}", ""]
        if not self.issues:
...
SECRET_PATTERNS = [
    ...
    re.compile(r"(?i)CHAT-E2E-20260430-QUALITY-[A-Za-z0-9_-]*(password|secret)[A-Za-z0-9_-]*"),
